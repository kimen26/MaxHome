...
export function creerUiTaches(api, etat, cb, ouvrirAjout) {
  ...
  // Un jour ne montre que SES occurrences : celle d'hier non faite vit sur l'onglet d'hier
  // (bande des 7 jours). L'inclure ici doublait chaque ligne quotidienne sans dire pourquoi
  // (« Biberons » deux fois, 0/14), ce que la vraie base a montré et pas les données factices.
  const duJour = (jour) => aFaire().filter((t) => groupe(t, jour) === "aujourdhui");
   ...
   *  règle que la ligne de carte (ligneTache, ui-taches-cartes.js), lue ici pour le détail. */
  ...
  function bandeJours() {
    ...
    return `<div class="bande-jours">${jours.map((j) => {
      ...
      return `<button type="button" class="${classes} cible44" data-jour="${j}" aria-label="${txt(aria)}">
  ...
  function rendre() {
    ...
    // Préfixe relatif au jour SÉLECTIONNÉ (pas toujours « aujourd'hui », cf. libelleRelatif —
    // bug 1 du lot : le sous-titre restait figé sur « aujourd'hui » quel que soit le jour choisi).
    const relatif = libelleRelatif(jour, jourIso(new Date()));
    const surAuj = jour === jourIso(new Date());
    ...
    $("#sous-jour").textContent = oblRestants.length
      ? `${relatif}oblig. : ${apercuObl}`
      : restantes.length ? `${relatif}reste ${restantes.length} tâche${restantes.length > 1 ? "s" : ""}`
        : termines.length ? `${relatif}${surAuj ? "rien oublié aujourd’hui" : "rien oublié ce jour-là"}`
          : `${relatif}rien de prévu`;
    ...
    $("#cartes-periode").innerHTML = `<div class="grille-cartes">
      ${cartePeriode("Cette semaine", "hebdo", jour, ctxCartes)}${cartePeriode("Ce mois", "mensuel", jour, ctxCartes)}
    </div>`;
    ...
    $("#barre-faites").innerHTML = `<button type="button" class="barre-fait-bouton" id="bouton-plier-faites"
      ...
      <span class="fait-pastilles">${termines.slice(0, 6).map((t) => {
        // La lettre de la personne, pas les parts (bug 3) : la couleur seule ne suffit jamais,
        // la lettre porte l'info (rules/mobile-parents.md, contraste).
        const libelle = t.qui2 ? "CY" : t.qui ? t.qui[0].toUpperCase() : "";
        return `<span class="pastille-fait ${t.qui2 ? "case-cycle-deux" : t.qui === p1 ? "case-cycle-p1" : "case-cycle-p2"}">${txt(libelle)}</span>`;
      }).join("")}</span>
    ...
    if (faitesOuvertes) {
      $("#liste-faites-jour").innerHTML = termines.length
        ? `<div class="carte-taches">${termines.map((t) => ligneTache(t, ctxCartes)).join("")}</div>`
   ...
   *  méta 11 px (« fait par Claudia » si une occurrence récente existe, sinon « ajouté il y a
   *  N j » via `cree_le`/libelleAjoutTodo — bug 6, 014_cree_le.sql), bouton en tirets pour en
   *  ajouter. */
  function ouvrirTodo() {
    ...
    const ligne = (r) => {
      ...
      const meta = fait ? `fait par ${txt(derniere.qui)}${derniere.qui2 ? ` et ${txt(derniere.qui2)}` : ""}`
        : txt(libelleAjoutTodo(r.cree_le, r.minutes, new Date()));
